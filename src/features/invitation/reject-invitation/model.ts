...
const handlerFx = createEffect(invitationsApi.rejectInvitation);
...
sample({
	...
	fn: () =>
		({
			message: i18n.t('actions.reject.notifications.success', {
				ns: 'room-invitation',
			}),
			color: 'success',
		} as const),
...
sample({
	...
	fn: () =>
		({
			message: i18n.t('actions.reject.notifications.error', {
				ns: 'room-invitation',
			}),
			color: 'error',
		} as const),
